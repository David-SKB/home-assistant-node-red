...
`template:
  - sensor:
      - name: "Motion Lighting Auto Context Window ${area_name}"
        ...
        attributes:
          ms: >
            {% set uom = state_attr('input_number.motion_lighting_auto_context_window_${area_id}', 'unit_of_measurement') | lower %}
            {% set value = states('input_number.motion_lighting_auto_context_window_${area_id}') | float %}
            {% if uom == 'minutes' %}
              {{ value * 60000 }}
            {% elif uom == 'seconds' %}
              {{ value * 1000 }}
          ...
          seconds: >
            {% set ms = state_attr('sensor.motion_lighting_auto_context_window_${area_id}', 'ms') | float %}
            {{ ms / 1000 }}
