...
template:
  - sensor:
      - name: "Motion Lighting Auto Maximum Timeout ${area_name}"
        ...
        attributes:
          ms: >
            {% set uom = state_attr('input_number.motion_lighting_auto_maximum_timeout_${area_id}', 'unit_of_measurement') %}
            {% set value = states('input_number.motion_lighting_auto_maximum_timeout_${area_id}') %}
