...
describe('stripCharacter', () => {
  ...
  it('should handle strings that are entirely the specified character', () => {
    expect(stripCharacter('****', '*')).toBe('');
    expect(stripCharacter('######', '#')).toBe('');
    expect(stripCharacter('!!!!!!', '!')).toBe('');
  });
  ...
  it('should throw an error when str is undefined', () => {
    try {
        stripCharacter(undefined);
      } catch (error) {
        expect(error).toBeInstanceOf(TypeError);
        expect(error.message).toBe("Error in stripCharacter: Cannot read properties of undefined (reading 'replace')");
      }
  });
});